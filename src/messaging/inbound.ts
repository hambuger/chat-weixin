...
/** Inbound context passed to the chat-weixin pipeline (matches MsgContext shape). */
export type WeixinMsgContext = {
  Body: string;
  From: string;
  To: string;
  AccountId: string;
  OriginatingChannel: "chat-weixin";
  OriginatingTo: string;
  MessageSid: string;
  Timestamp?: number;
  Provider: "chat-weixin";
  ChatType: "direct";
...
function bodyFromItemList(itemList?: MessageItem[]): string {
  ...
  for (const item of itemList) {
    ...
    // Voice transcript: if voice_item.text exists, use it as message body directly.

...
/**
 * Send a voice message downstream using a previously uploaded SILK payload.
 * VoiceItem: media (CDN ref), encode_type=6 (silk), optional playtime(ms).
 * Includes an optional text caption sent as a separate TEXT item first.
 */
export async function sendVoiceMessageWeixin(params: {
  ...
  const { to, text, uploaded, durationMs, opts } = params;
  if (!opts.contextToken) {
  ...
  const voiceItem: MessageItem = {
    ...
    voice_item: {
      ...
      encode_type: 6, // silk
      sample_rate: 24_000,
      playtime: durationMs && durationMs > 0 ? Math.floor(durationMs) : undefined,
    },
  };

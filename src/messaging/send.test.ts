...
describe("sendVoiceMessageWeixin", () => {
  it("sends voice message with optional playtime", async () => {
    mockSendMessageApi.mockResolvedValue(undefined);
    ...
    expect(result.messageId).toBeDefined();
    const callArgs = mockSendMessageApi.mock.calls[0][0];
    expect(callArgs.body.msg.item_list[0].type).toBe(3);
    expect(callArgs.body.msg.item_list[0].voice_item.encode_type).toBe(6);
    expect(callArgs.body.msg.item_list[0].voice_item.playtime).toBe(1333);
  });
});

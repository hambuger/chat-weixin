...
vi.mock("../cdn/upload.js", () => ({
  ...
  uploadVoiceBufferToWeixin: mockUploadVoiceBufferToWeixin,
}));
...
const { mockAudioToSilk } = vi.hoisted(() => ({
  mockAudioToSilk: vi.fn(),
}));

vi.mock("../media/silk-transcode.js", () => ({
  audioToSilk: mockAudioToSilk,
}));
...
describe("sendWeixinMediaFile", () => {
  ...
  it("routes audio/* to SILK voice upload + sendVoiceMessageWeixin", async () => {
    mockAudioToSilk.mockResolvedValueOnce({ silk: Buffer.from("silk"), durationMs: 1200 });
    mockUploadVoiceBufferToWeixin.mockResolvedValueOnce(fakeUploaded);
    mockSendVoiceMessageWeixin.mockResolvedValueOnce({ messageId: "voice1" });
    const result = await sendWeixinMediaFile({ ...baseParams, filePath: "/tmp/voice.wav" });
    expect(result.messageId).toBe("voice1");
    expect(mockAudioToSilk).toHaveBeenCalledOnce();
    expect(mockUploadVoiceBufferToWeixin).toHaveBeenCalledOnce();

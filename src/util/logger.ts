...
/** Logger writes JSON lines to <tmpDir>/chat-weixin-YYYY-MM-DD.log. */

const MAIN_LOG_DIR = process.env.WEIXIN_LOG_DIR?.trim() || os.tmpdir();
const SUBSYSTEM = "chat-weixin/bridge";
const RUNTIME = "node";
const RUNTIME_VERSION = process.versions.node;
const HOSTNAME = os.hostname() || "unknown";
const PARENT_NAMES = ["chat-weixin"];
...
function resolveMinLevel(): number {
  const env = (
    process.env.WEIXIN_LOG_LEVEL ||
    process.env.CHAT_WEIXIN_LOG_LEVEL
  )?.toUpperCase();
...
function resolveMainLogPath(): string {
  ...
  return path.join(MAIN_LOG_DIR, `chat-weixin-${dateKey}.log`);
}

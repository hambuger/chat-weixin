...
describe("getExtensionFromMime", () => {
  it("returns correct extension for known MIME types", () => {
    ...
    expect(getExtensionFromMime("audio/silk")).toBe(".silk");
  });
  ...
  it("maps wildcard media MIME types to practical defaults", () => {
    expect(getExtensionFromMime("image/*")).toBe(".jpg");
    expect(getExtensionFromMime("video/*")).toBe(".mp4");
    expect(getExtensionFromMime("audio/*")).toBe(".wav");
  });
});

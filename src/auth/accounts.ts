...
function resolveWeixinStateDir(): string {
  return path.join(resolveStateDir(), "chat-weixin");
}
...
/**
 * Legacy single-file token: `credentials/chat-weixin/credentials.json` (pre per-account files).
 */
function loadLegacyToken(): string | undefined {
  const legacyPath = path.join(resolveStateDir(), "credentials", "chat-weixin", "credentials.json");
  try {
 ...
 *   - credentials/chat-weixin-{accountId}-allowFrom.json (authorized users)
 */
 ...
 * Checks WEIXIN_CONFIG first, then state dir.
 */
function resolveConfigPath(): string {
  const envPath = process.env.WEIXIN_CONFIG?.trim();
  if (envPath) return envPath;
  return path.join(resolveStateDir(), "chat-weixin.json");
}

/**
 * Read `routeTag` from config file (for callers without a full config object).
 * Checks per-account `channels.<id>.accounts[accountId].routeTag` first, then section-level
 * `channels.<id>.routeTag`. Matches `feat_weixin_extension` behavior; channel key is `"chat-weixin"`.
 *
...
function loadRouteTagSection(): Record<string, unknown> | null {
  ...
  try {
    ...
    const section = (channels?.["chat-weixin"] as Record<string, unknown>) ?? null;
...
/**
 * Bump `channels.chat-weixin.channelConfigUpdatedAt` in config on each successful login
...
export async function triggerWeixinChannelReload(): Promise<void> {
  logger.info("triggerWeixinChannelReload: noop in standalone mode");
}
...
type WeixinAccountConfig = {
  ...
  enabled?: boolean;
  cdnBaseUrl?: string;
  /** Optional SKRouteTag source; read from chat-weixin config when `accountId` is passed to `loadConfigRouteTag`. */
...
export function listWeixinAccountIds(_cfg?: unknown): string[] {
...
export function resolveWeixinAccount(
  ...
  const section = cfg.channels?.["chat-weixin"] as WeixinSectionConfig | undefined;

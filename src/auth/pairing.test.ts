...
describe("resolveFrameworkAllowFromPath", () => {
  it("returns correct path for a given accountId", async () => {
    ...
    expect(result).toBe(
      path.join(tmpDir, "credentials", "chat-weixin-test-account-allowFrom.json"),
    );
  });
  ...
  it("respects WEIXIN_CREDENTIALS_DIR override", async () => {
    ...
    expect(result).toBe(path.join(customDir, "chat-weixin-my-bot-allowFrom.json"));
  });
  ...
  it("sanitizes special characters in accountId", async () => {
    ...
    expect(result).toContain("chat-weixin-abc@im.bot-allowFrom.json");
  });
});
...
describe("registerUserInFrameworkStore", () => {
  ...
  it("cleans lock file after write", async () => {
    ...
    const lockPath = `${resolveFrameworkAllowFromPath("acc6")}.lock`;

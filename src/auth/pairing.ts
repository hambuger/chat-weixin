 ...
 * Path priority:
 *   1) $WEIXIN_CREDENTIALS_DIR
 *   2) <stateDir>/credentials
 */
function resolveCredentialsDir(): string {
  const override = process.env.WEIXIN_CREDENTIALS_DIR?.trim();
 ...
 * Path: `<credDir>/chat-weixin-<accountId>-allowFrom.json`
 */
export function resolveFrameworkAllowFromPath(accountId: string): string {
  const base = safeKey("chat-weixin");
...
async function withFileLock<T>(filePath: string, _options: unknown, fn: () => Promise<T>): Promise<T> {

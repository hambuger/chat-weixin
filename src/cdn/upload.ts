...
async function uploadBytesToCdn(params: {
  plaintext: Buffer;
  toUserId: string;
  ...
  const { plaintext, toUserId, opts, cdnBaseUrl, mediaType, label, sourceLabel } = params;
  ...
  logger.debug(
    `${label}: src=${sourceLabel ?? "(buffer)"} rawsize=${rawsize} filesize=${filesize} md5=${rawfilemd5} filekey=${filekey}`,
  );
...
/** Upload a SILK voice payload buffer to the Weixin CDN. */
export async function uploadVoiceBufferToWeixin(params: {
  silk: Buffer;
  toUserId: string;
  opts: WeixinApiOptions;
  cdnBaseUrl: string;
  ...
  return uploadBytesToCdn({
    plaintext: params.silk,
    toUserId: params.toUserId,
    opts: params.opts,
    cdnBaseUrl: params.cdnBaseUrl,
    mediaType: UploadMediaType.VOICE,
    label: "uploadVoiceBufferToWeixin",
    sourceLabel: "(silk-buffer)",
  });
}

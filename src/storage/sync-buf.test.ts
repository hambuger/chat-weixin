...
describe("getSyncBufFilePath", () => {
  it("returns path under accounts dir", async () => {
    ...
    expect(result).toBe(path.join(tmpDir, "chat-weixin", "accounts", "myacc.sync.json"));
  });
});
...
describe("loadGetUpdatesBuf", () => {
  ...
  it("falls back to compat path for -im-bot suffix", async () => {
    ...
    const dir = path.join(tmpDir, "chat-weixin", "accounts");
  ...
  it("falls back to legacy sync buf path", async () => {
    ...
    const legacyDir = path.join(tmpDir, "agents", "default", "sessions", ".chat-weixin-sync");

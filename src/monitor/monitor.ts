...
import { forwardInboundMessageToChatService } from "../bridge/inbound-forward.js";
import { CHAT_CALLBACK_URL } from "../bridge/http-routes.js";
...
/** Long-poll loop: getUpdates -> forward inbound messages to local chat service. */
  ...
  while (!abortSignal?.aborted) {
    try {
      ...
      for (const full of list) {
        ...
        try {
          await forwardInboundMessageToChatService({
            accountId,
            cdnBaseUrl,
            message: full,
          });
          aLog.info(`inbound message forwarded to ${CHAT_CALLBACK_URL}`);
        } catch (err) {
          aLog.error(`forward inbound failed: ${String(err)}`);
        }
      }
    } catch (err) {
